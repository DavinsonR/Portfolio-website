# Borrador — `/[lang]/historia`

> **Esto es un borrador para que lo leas y lo corrijas, no está publicado.**
> Cada cifra sale de `lib/content/`, de la bitácora o de los repositorios: no
> inventé ninguna. Lo que sí es mío es el orden y el tono — dime qué suena a ti
> y qué no, y lo reescribo antes de convertirlo en página.
>
> Marco con **[VERIFICA]** las tres frases que no puedo comprobar solo.
...
**26 meses**. Entre medias, reconocimiento de ingresos en SAP bajo Sarbanes-Oxley
y auditorías internas sobre proyectos de tecnología. Ahí aprendí algo que no
estaba en el temario: **un número que no se puede auditar no vale nada**, por
bueno que sea. Esa frase reaparece al final de esta página, y no por casualidad.
...
**[VERIFICA] 1.** Doy por hecho que tu puesto actual es **Neoris/EPAM, Business
Consultant FP&A**, como dice el CV. Si empezaste en **Precia PPV S.A.**, esta
página y el CV cambian — dime y lo ajusto todo de una vez.

**[VERIFICA] 2.** «Entré a la tesorería de SLB como practicante en enero de
2024» sale de las fechas del CV (Treasury Intern, Jan 2024 — Jun 2024). Si el
orden real fue otro, corrígeme.

**[VERIFICA] 3.** La frase sobre las auditorías internas de SLB y el gate que
bloquea tu modelo la conecté yo. Es una lectura mía de tu trayectoria, y es la
mejor idea de la página — pero solo si es cierta para ti. Si te suena forzada,
la quito.

**Decisiones que tomé y puedes revertir:**

- **Primera persona.** El resto del sitio es tercera persona implícita; una
  página de historia en tercera persona suena a biografía institucional.
- **Sin cifras nuevas.** Todo sale de `lib/content/`, la bitácora o los repos.
- **El orden es cronológico salvo el cierre.** La sección 6 rompe la cronología
  a propósito: es la tesis del texto y va al final, no al principio.
- **No menciono JARVIS.** Es privado y la historia ya tiene cinco hitos; meter
  un sexto la diluye. Si quieres que entre, dime dónde.

**Cuando me digas que va, hago esto:**

1. Traduzco al inglés con el mismo cuidado (no es una traducción literal: el
   inglés de este sitio es más seco).
2. Bloque `historia` en `lib/content/about.ts`, en los dos idiomas.
3. `app/[lang]/historia/page.tsx` con `alternates` y `openGraph` propios.
4. `ROUTES` de `app/sitemap.ts` y el redirect de `/historia` en `next.config.ts`.
5. El enlace desde la portada y desde la barra.
6. `npm run check` y `npm run check:routes`, que ya vigilan que no falte ninguno
   de esos cinco pasos.
